...
class MercadoPagoService:
    ...
    def __init__(self):
        ...
        if not settings.MP_ACCESS_TOKEN or settings.MP_ACCESS_TOKEN.strip() == '':
            raise ValueError('A variável de ambiente MP_ACCESS_TOKEN não foi definida ou está vazia.')
        
        if not settings.MP_BASE_API_URL or settings.MP_BASE_API_URL.strip() == '':
            raise ValueError('A variável de ambiente MP_BASE_API_URL não foi definida ou está vazia.')
        
        if not settings.NOTIFICATION_URL or settings.NOTIFICATION_URL.strip() == '':
            raise ValueError('A variável de ambiente NOTIFICATION_URL não foi definida ou está vazia.')
    ...
    def generate_payment_expiration_date(self, days: int | None = None, hours: int | None = None, minutes: int | None = None):
        ...
        try:
            ...
            raise ValueError(f'Timezone "{settings.TIME_ZONE}" não encontrado. Verifique a configuração.')
    ...
    def pay_with_pix(self, amount: float, payer_email: str, payer_cpf: str, description: str = 'Pagamento'):
        ...
        try:
            payload = {
                ...
                'payer': {'email': payer_email.strip(), 'identification': {'type': 'CPF', 'number': payer_cpf.replace('.', '').replace('-', '')}},
    ...
    def pay_with_boleto(
        ...
        if not isinstance(payer_address, dict) or not payer_address:
            raise ValueError('Endereço do pagador é obrigatório e deve ser um dicionário.')
        
        required_address_fields = ['zip_code', 'street_name', 'neighborhood', 'city', 'federal_unit']
        ...
        try:
            payload = {
                ...
                'payer': {
                    ...
                    'address': {
                        ...
                        'street_number': payer_address.get('street_number', 'S/N').strip(),
                        'neighborhood': payer_address.get('neighborhood').strip(),
                        'city': payer_address.get('city').strip(),
                        'federal_unit': payer_address.get('federal_unit').strip().upper(),
                    },
                },
            ...
            if isinstance(e, (ValueError, RuntimeError)):
                raise
            raise RuntimeError(f'Erro inesperado ao criar pagamento por boleto: {str(e)}')
    ...
    def pay_with_card(self, amount: float, payer_email: str, payer_cpf: str, card_data: dict, installments: int = 1, description: str = 'Pagamento'):
        ...
        if not isinstance(card_data, dict) or not card_data:
            raise ValueError('Dados do cartão são obrigatórios e devem ser um dicionário.')
        
        required_card_fields = ['card_number', 'expiration_month', 'expiration_year', 'security_code', 'cardholder']
        ...
        try:
            ...
            if isinstance(e, (ValueError, RuntimeError)):
                raise
            raise RuntimeError(f'Erro inesperado ao criar pagamento com cartão: {str(e)}')
    ...
    def get_payment_info(self, transaction_id: str):
        ...
        try:
            ...
            if isinstance(e, (ValueError, RuntimeError)):
                raise
            raise RuntimeError(f'Erro inesperado ao buscar informações do pagamento: {str(e)}')
    ...
    def _post(self, path: str, payload: dict, use_idempotency_key: bool = True):
        ...
        try:
            response = requests.post(url=url, headers=headers, json=payload, timeout=30.0)
    ...
    def _get_card_token(self, card_data: dict):
        ...
        """
        try:
            token_response = self._post('/v1/card_tokens', card_data, use_idempotency_key=False)
