...
def calculate_metrics():
    ...
    revenue_last_7_days = sum(order.total_price for order in Order.objects.filter(created_at__gte=last_7_days))

    # Vendas nos últimos 30 dias
    last_30_days = now() - timedelta(days=30)
    sales_last_30_days = Order.objects.filter(created_at__gte=last_30_days).count()
    revenue_last_30_days = sum(order.total_price for order in Order.objects.filter(created_at__gte=last_30_days))
