...
class MetricsTestSuite:
    ...
    def create_order(self, customer_name, phone, address, status, payment_status, 
                    payment_method='pix', cash_value=None, items=None, created_at=None):
        """
    ...
    def create_test_scenario(self):
        ...
        order1, value1 = self.create_order(
            "TESTE_Cliente_1", "11999999001", "Rua A, 123",
        ...
        order2, value2 = self.create_order(
            "TESTE_Cliente_2", "11999999002", "Rua B, 456", 
            status="completed", payment_status="paid",
            items=[
        ...
        order3, value3 = self.create_order(
            "TESTE_Cliente_3", "11999999003", "Rua C, 789",
        ...
        order4, value4 = self.create_order(
            "TESTE_Cliente_4", "11999999004", "Rua D, 101",
        ...
        order5, value5 = self.create_order(
            "TESTE_Cliente_5", "11999999005", "Rua E, 202",
        ...
        order6, value6 = self.create_order(
            "TESTE_Cliente_6", "11999999006", "Rua F, 303",
            status="pending", payment_status="pending",
            items=[
                {'product_index': 0, 'quantity': 1},  # 1x Água 20L = 15.00
            ],
        ...
        order7, value7 = self.create_order(
            "TESTE_Cliente_7", "11999999007", "Rua G, 404",
            status="completed", payment_status="paid",
            items=[
        ...
        order8, value8 = self.create_order(
            "TESTE_Cliente_8", "11999999008", "Rua H, 505",
            status="completed", payment_status="paid", 
            items=[
        ...
        order9, value9 = self.create_order(
            "TESTE_Cliente_9", "11999999009", "Rua I, 606",
            status="completed", payment_status="paid",
            items=[
        ...
        order10, value10 = self.create_order(
            "TESTE_Cliente_10", "11999999010", "Rua J, 707",
            status="completed", payment_status="paid",
            items=[
                {'product_index': 0, 'quantity': 1},  # 1x Água 20L = 15.00
                {'product_index': 1, 'quantity': 1},  # 1x Água 10L = 8.50  
        ...
        order11, value11 = self.create_order(
            "TESTE_Cliente_11", "11999999011", "Rua K, 808",
            status="completed", payment_status="paid",
            items=[
        ...
        self.test_data = {
            ...
            # RECEITAS DO DIA
            'expected_revenue_paid_today': float(value1 + value2 + value4),  # orders 1, 2, 4 
            'expected_revenue_pending_today': float(value3 + value6),  # orders 3, 6
            'expected_revenue_cancelled_today': float(value5),  # order 5
            'expected_revenue_today': float(value1 + value2 + value3 + value4 + value6),  # exclui cancelado
            
            # MÉTRICAS GERAIS
            'expected_total_effective_sales': 6,  # orders 1, 2, 7, 8, 9, 10, 11
            'expected_total_effective_revenue': float(value1 + value2 + value7 + value8 + value9 + value10 + value11),
            
            # ÚLTIMOS 7 DIAS (inclui hoje)
            'expected_effective_sales_last_7_days': 4,  # orders 1, 2, 7, 8
            'expected_effective_revenue_last_7_days': float(value1 + value2 + value7 + value8),
            
            # ÚLTIMOS 30 DIAS (inclui hoje)
            'expected_effective_sales_last_30_days': 6,  # orders 1, 2, 7, 8, 9, 10
            'expected_effective_revenue_last_30_days': float(value1 + value2 + value7 + value8 + value9 + value10),
            
            # PEDIDOS ATRASADOS GLOBAIS
            'expected_late_orders_count': 1,  # order 6 apenas
        }
        
        print(f"✅ Cenário criado: {Order.objects.filter(customer_name__startswith='TESTE_').count()} pedidos")
    ...
    def validate_metrics(self):
        ...
        validations = [
            ...
            ('effective_revenue_last_30_days', 'expected_effective_revenue_last_30_days'),
