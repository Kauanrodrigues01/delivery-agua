...
class Command(BaseCommand):
    ...
    def cleanup_demo_orders(self):
        ...
        self.stdout.write(
            self.style.WARNING('🧹 Removendo pedidos de demonstração...')
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'✅ {deleted_count} pedidos de demonstração removidos')
            )
    ...
    def create_demo_orders(self, specific_count=None):
        ...
        if not products:
            raise CommandError('❌ Nenhum produto ativo encontrado! Crie produtos primeiro.')
        ...
        try:
            with transaction.atomic():
                ...
                # ===== DADOS PARA DEMO =====
                customers = [
                    {"name": "Maria Silva", "phone": "11987654321", "address": "Rua das Flores, 123 - Centro"},
                    {"name": "João Santos", "phone": "11976543210", "address": "Av. Principal, 456 - Jardim América"},
                    {"name": "Ana Costa", "phone": "11965432109", "address": "Rua do Comércio, 789 - Vila Nova"},
                    {"name": "Carlos Oliveira", "phone": "11954321098", "address": "Alameda dos Pássaros, 321 - Alto da Boa Vista"},
                    {"name": "Lucia Ferreira", "phone": "11943210987", "address": "Rua da Paz, 654 - Centro"},
                    {"name": "Pedro Almeida", "phone": "11932109876", "address": "Av. das Nações, 987 - Jardim Europa"},
                    {"name": "Sandra Lima", "phone": "11921098765", "address": "Rua Nova Esperança, 159 - Vila Progresso"},
                    {"name": "Roberto Dias", "phone": "11910987654", "address": "Alameda Central, 753 - Bela Vista"},
                    {"name": "Fernanda Rocha", "phone": "11909876543", "address": "Rua do Sol, 951 - Jardim Primavera"},
                    {"name": "Marcos Pereira", "phone": "11898765432", "address": "Av. da Liberdade, 357 - Centro Histórico"},
                ]
                ...
                if specific_count:
                    ...
                    for i in range(specific_count):
                        ...
                        if date_rand <= 0.5:  # 50% hoje - boa concentração no mesmo dia
                            days_ago = 0
                            hours_range = (7, 21)  # Horário estendido para mais variedade
                        elif date_rand <= 0.8:  # 30% últimos 3 dias - concentração recente
                            days_ago = random.randint(1, 3)
                            hours_range = (8, 19)
                        elif date_rand <= 0.95:  # 15% últimos 15 dias - distribuição média
                        ...
                        if days_ago == 0:
                            ...
                            if days_ago <= 3:
                                ...
                                hour_options = list(range(*hours_range)) + [10, 14, 16, 18]  # Reforçar alguns horários
                        ...
                        is_late = False
                        if days_ago == 0 and selected_status == "pending" and random.random() < 0.4:
                            # 40% dos pendentes de hoje serão atrasados (aumentei de 30% para 40%)
                            late_minutes = random.choice([25, 30, 45, 60, 90, 120, 150, 180])
                        ...
                        if selected_status == "completed" and selected_payment_status == "paid":
                            total_revenue += order_value
                        
                        orders_created += 1
                        
                        # Progress indicator com info da data
                        if (i + 1) % 5 == 0 or i < 5:  # Mostrar mais frequentemente no início
                            self.stdout.write(f'   📊 {i + 1}/{specific_count} pedidos criados | Último: {order_date.strftime("%d/%m %H:%M")} ({days_ago} dias atrás)')
                        ...
                        if (i + 1) % 10 == 0:
                            ...
                            self.stdout.write(f'   📊 {i + 1}/{specific_count} pedidos criados ({today_count} hoje)')
                    ...
                    self.stdout.write(f'\n📈 Distribuição criada:')
                    self.stdout.write(f'   - HOJE: {today_count} pedidos (completados: {today_completed}, pendentes: {today_pending}, cancelados: {today_cancelled})')
                    self.stdout.write(f'   - Últimos 3 dias: {last_3_days_count} pedidos')
                    self.stdout.write(f'   - Últimos 7 dias: {last_7_days_count} pedidos')
                    self.stdout.write(f'   - Últimos 30 dias: {last_30_days_count} pedidos')
                    
                    # Porcentagem de concentração no dia atual
                    concentration_today = (today_count / specific_count * 100) if specific_count > 0 else 0
                    self.stdout.write(f'   - Concentração hoje: {concentration_today:.1f}% dos pedidos criados')
                    ...
                    for i in range(12):
                        ...
                        hour_weights = [7, 8, 9, 9, 10, 10, 11, 12, 12, 12, 13, 14, 14, 15, 15, 16, 17, 17, 18, 18, 19, 20]
                    ...
                    late_times = [25, 30, 45, 60, 75, 90, 120, 150, 180, 240]  # mais opções de atraso
                    for i in range(3):
                        customer = random.choice(customers)
                        ...
                        order = Order.objects.create(
                            ...
                            created_at=now - timedelta(minutes=minutes_ago, seconds=random.randint(0, 59))
                        )
                    ...
                    for i in range(3):  # aumentei de 2 para 3
                        customer = random.choice(customers)
                    ...
                    self.stdout.write(self.style.SUCCESS(f'✅ Pedidos de hoje criados'))
                    ...
                    for day in range(1, 4):
                        ...
                        for i in range(num_orders_day):
                            customer = random.choice(customers)
                            
                            # Horários variados com picos
                            hour_options = [8, 9, 10, 10, 11, 12, 12, 13, 14, 14, 15, 16, 17, 17, 18, 19]
                            selected_hour = random.choice(hour_options)
                            ...
                            order = Order.objects.create(
                                ...
                                payment_method=random.choice(["pix", "dinheiro", "cartao"]),
                                created_at=target_date + timedelta(
                                    hours=selected_hour, 
                                    minutes=random.randint(0, 59),
                                    seconds=random.randint(0, 59)
                                )
                    ...
                    for day in range(8, 31):
                        ...
                        if day % 5 == 0:
                            self.stdout.write(f'   Dia -{day}: processado ({num_orders_day} pedidos)')
                    
                    self.stdout.write(self.style.SUCCESS('✅ Pedidos dos últimos 30 dias criados'))
                ...
                concentration_percentage = (final_today_count / final_total_count * 100) if final_total_count > 0 else 0
                
                self.stdout.write(f'📊 Concentração no dia atual: {final_today_count}/{final_total_count} pedidos ({concentration_percentage:.1f}%)')
